...
with open("2024/Day 20/input.txt", 'r') as file:
...
def get_cheat_dists(path, pos, node, index):
...
def part1(path, goal):
    ...
    for pos in path:
        adj = get_adj(pos, '#')
        for node in adj:
            dists = get_cheat_dists(path, pos, node, index)
            for dist in dists:
                if dist <= goal:
                    num_cheats += 1
        index += 1
